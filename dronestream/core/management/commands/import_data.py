...
def parse_country(strike_country):
    obj, created = Country.objects.get_or_create(name=strike_country.capitalize())
    return obj


def parse_province(country, strike_province):
...
def parse_town(province, strike_town):
    ...
    def handle(self, *args, **options):
        ...
        for strike_json in dronestream_json["strike"]:
            ...
            try:
                ...
                strike.date = parse_date(strike_json["date"])
                strike.narrative = strike_json["narrative"]
                strike.country = parse_country(strike_json["country"])
                strike.province = parse_province(strike.country, strike_json["location"])
                strike.town = parse_town(strike.province, strike_json["town"])
                strike.deaths_min = parse_int(strike_json["deaths_min"])
                strike.deaths_max = parse_int(strike_json["deaths_max"])
                strike.civilians_min, strike.civilians_max = parse_int_range(strike_json["civilians"])
                strike.injuries_min, strike.injuries_max = parse_int_range(strike_json["injuries"])
                strike.children_min, strike.children_max = parse_int_range(strike_json["children"])
                strike.tweet_id = parse_int(strike_json["tweet_id"])
                ...
                strike.latitude = parse_geocoord(strike_json["lat"])
                strike.longitude = parse_geocoord(strike_json["lon"])
                strike.names = parse_names(strike_json["names"])
                strike.save()
